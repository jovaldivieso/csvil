...
class SimulatorContractTests(unittest.TestCase):
    ...
    def test_multi_robot_casadi_dynamics_falls_back_for_heterogeneous_fleet(self) -> None:
        simulator = DynamicsFactory.create(
            system_name="multi_robot",
            config={
                "dt": 0.05,
        ...
        result = simulator.casadi_dynamics(ca.DM(state), ca.DM(action))

        self.assertEqual(int(result.shape[0]), int(simulator.nx))
        self.assertIsNone(simulator._casadi_mapped_dynamics)
